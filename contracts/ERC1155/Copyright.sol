...
contract Copyright is ERC1155, IERC2981 {
    ...
    function supportsInterface(bytes4 interfaceId) public view virtual override(ERC1155, IERC165, IERC2981) returns (bool) {
    ...
    function royaltyInfo(uint256 tokenId, uint256 value) external view override returns (address receiver, uint256 royaltyAmount) {
        uint256 totalRoyalties = value * _royalties[tokenId] / MAX_ROYALTY_FEE;
        return (BEMA_CHAIN_ADDRESS, totalRoyalties);
    }

    function setRoyalties(uint256[] memory tokenIds, uint256[] memory royalties) external {
